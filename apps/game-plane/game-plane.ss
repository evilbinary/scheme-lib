...
(define (game)
  (glut-init
   (lambda()
     (case (machine-type)
        ((arm32le) (glut-init-window-size 600 1800) )
        (else (glut-init-window-size 600 600))
         )
  ...
  (glut-reshape
   (lambda(w h)
     ...
     (if fixed
        (glViewport 0 0 fixed-w fixed-h )
        (glViewport 0 0 (* w scale-w) (* h scale-h) ))
