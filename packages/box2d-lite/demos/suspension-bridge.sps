...
(window (size 800 800)
	(title "Box2d Lite - Suspension Bridge")
	(reshape (width height)
		 (lambda (w h)
		   (glMatrixMode GL_PROJECTION)
		   (glLoadIdentity)
		   (glOrtho -10.0 10.0 -5.0 15.0 -1000.0 1000.0))))
...
(let ((num-planks 15)
  ...
  (let ((b (create-body)))
    ...
    (do ((i 0 (+ i 1)))
      ...
      (b.set (make-vec 1.0 0.25) mass)

      (b.friction! 0.2)

      (b.position! (make-vec (+ -8.5 (* 1.25 i)) 5.0))
  ...
  (let ((j (create-joint)))
    ...
    (let ((frequency-hz  2.0)
          (damping-ratio 0.7))

      (let ((omega (* 2.0 pi frequency-hz)))

        (let ((d (* 2.0 mass damping-ratio omega)))

          (let ((k (* mass omega omega)))

            (let ((softness (/ 1.0 (+ d (* time-step k)))))

              (let ((bias-factor (/ (* time-step k) (+ d (* time-step k)))))

                (let ((bodies (list->vector world.bodies)))

                  (do ((i 0 (+ i 1)))
                      ((>= i num-planks))

                    (j.set (vector-ref bodies i)
                           (vector-ref bodies (+ i 1))
                           (make-vec (+ -9.125 (* 1.25 i)) 5.0))

                    (j.softness! softness)

                    (j.bias-factor! bias-factor)

                    (world.add-joint j)

                    (set! j (create-joint)))

                  (j.set (vector-ref bodies num-planks)
                         (vector-ref bodies 0)
                         (make-vec (+ -9.125 (* 1.25 num-planks)) 5.0))
