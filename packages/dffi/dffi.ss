...
(library (dffi dffi)
  (export
   ...
   dffi-get-pointer
   dffi-get-string
   dffi-get-string-offset
   dffi-set-pointer
   dffi-set-string
   dffi-get-pointer-offset
   dffi-get-string-pointer
   dffi-string-pointer
   dffi-string
   print-ptr
   print-string
  ...
  (define dcNewCallVM (foreign-procedure "dcNewCallVM" (int ) void*))
  (define dcFree (foreign-procedure "dcFree" (void*) void))
  (define dcReset (foreign-procedure "dcReset" (void*) void))

  (define dcMode (foreign-procedure "dcMode" (void* int) void))

  (define dcArgBool (foreign-procedure "dcArgBool" (void* int) void))
  (define dcArgChar (foreign-procedure "dcArgChar" (void* char) void))
  (define dcArgShort (foreign-procedure "dcArgShort" (void* short) void))
  (define dcArgInt (foreign-procedure "dcArgInt" (void* int) void))
  (define dcArgLong (foreign-procedure "dcArgLong" (void* int) void))
  (define dcArgLongLong (foreign-procedure "dcArgLongLong" (void* integer-64) void))
  (define dcArgFloat (foreign-procedure "dcArgFloat" (void* float) void))
  (define dcArgDouble (foreign-procedure "dcArgDouble" (void* double) void))
  (define dcArgPointer (foreign-procedure "dcArgPointer" (void* void*) void))
  (define dcArgStruct (foreign-procedure "dcArgStruct" (void* void* void*) void))
  (define dcArgString (foreign-procedure "dcArgPointer" (void* string) void))

  
  (define  dcCallVoid     (foreign-procedure "dcCallVoid"  (void*  void*) void))
  (define  dcCallBool     (foreign-procedure "dcCallBool"  (void*  void*) char))
  (define  dcCallChar     (foreign-procedure "dcCallChar"  (void*  void*) char))
  (define  dcCallShort    (foreign-procedure "dcCallShort"  (void*  void*) short))
  (define  dcCallInt      (foreign-procedure "dcCallInt"  (void*  void*) int))
  (define  dcCallLong     (foreign-procedure "dcCallLong"  (void*  void*) long))
  (define  dcCallLongLong (foreign-procedure "dcCallLongLong"  (void*  void*) integer-64))
  (define  dcCallFloat    (foreign-procedure "dcCallFloat"  (void*  void*) float))
  (define  dcCallDouble   (foreign-procedure "dcCallDouble"  (void* void*) double))
  (define  dcCallPointer  (foreign-procedure "dcCallPointer"  (void*  void*) void*))
  (define  dcCallStruct   (foreign-procedure "dcCallStruct"  (void* void* void* void*) void))
  (define  dcCallString  (foreign-procedure "dcCallPointer"  (void*  void*) string))

  (define dcGetError (foreign-procedure "dcGetError" (void*  void*)  int))


  (define dcNewStruct (foreign-procedure "dcNewStruct" (int int ) void*))
  (define dcStructField (foreign-procedure "dcStructField" (void* int int int ) void))
  (define dcSubStruct (foreign-procedure "dcSubStruct" (void* int int int ) void))

  (define dcCloseStruct (foreign-procedure "dcCloseStruct" (void* )  void))
  (define dcStructSize (foreign-procedure "dcStructSize" (void*)  int))
  (define dcStructAlignment (foreign-procedure "dcStructAlignment" (void* )  int))
  (define dcFreeStruct (foreign-procedure "dcFreeStruct" (void* ) void))
  ...
  (define-syntax try 
    ...
    (define test-call
      (foreign-procedure "test_call" () void*))
  ...
  (define (get-struct-field struct-val)
    (let ((ds (dcNewStruct (length (struct-ref struct-val 1))  DEFAULT_ALIGNMENT)))
      (printf "ds=>~a\n" ds)
      (let loop ((i 0)
	    ...
	    (case (car e)
	      ...
	      [(int64  long) 
	       (dcStructField ds DC_SIGCHAR_LONGLONG DEFAULT_ALIGNMENT 1)]
	      [(float ) 
  ...
  (define (dffi-call name funptr args ret values)
    (try
     (let ((alloc 0)
	   (vm (dcNewCallVM 10240)))
      (dcReset vm)
      (let loop ((arg args) (value values) (i 0))
          (if (pair? value)
              (begin 
                ;;(printf "  type=~a value=~a index=~a \n" (car arg) (car value) i   )
                (case (car arg)
		  ...
		  [(int64 long)  (dcArgLong vm (car value))]
		   ...
		   (if (string? (car value))
		       ...
		       (dcArgPointer vm (car value)))]
                  [else
		   (printf " %%%else type=~a size=~a \n" (car arg) (dffi-size (struct-ref (car value) 1) ) )
		   (let ((s (get-struct-field (car value))))
		     (set! s (get-struct))
		     (printf "s=>~a\n" s)
		     (set! alloc (ffi-alloc (dffi-size (struct-ref (car value) 1) ))  )
      ...
      (let ((ret-value
		...
		(let ((pret 0)
		  ...
		  (set! ret-struct-val
			((top-level-value 
			  (string->symbol (format "make-~a" ret) )) ))
		  (set! ret-type-s (struct-ref ret-struct-val 1) )
		  (set! s (get-struct-field ret-struct-val))
		  (set! s (get-struct))
		  
		  (set! pret (ffi-alloc (dffi-size ret-type-s )))
		  
		  (printf "  ###else type=~a ~a ~a\n"  ret ret-struct-val ret-type-s)
		  (printf "--->~d pret=>~a\n"  s pret)
		  (call-struct vm funptr s)
		  ;;(dcCallStruct vm funptr s  (ffi-get-pointer pret))
		  (printf "pret=>~d\n"  pret)
		  (set! vret (struct2lisp pret ret-struct-val))
		  (dcFreeStruct s)
		  vret
		  )
	...
	(if dffi-enable-log
    (printf "call ~a ~a ~a ~a ~a\ncall ret=~a\n" name funptr args ret values ret-value) )
	ret-value)
      )
    (catch (lambda (x)
  ...
  (define-syntax def-function
    (lambda (x)
      ...
      ;;(printf "~a\n>get loaded libs=>~a\n" x (get-loaded-libs-list))
      (syntax-case x ()
   	((_ name sym args ret )
	 (with-syntax ((libs (get-loaded-libs-list)))
	 #`(define name 
   	     (lambda values
   	       (dffi-call sym #,((lambda (libs s)
				   ;;(printf "get loaded libs=>~a\n" (get-loaded-libs-list))
			       (let loop ((lib libs))
				 (if (pair? lib)
				     (let ((ret (dlFindSymbol (dlLoadLibrary (car lib) )  (syntax->datum s) )))
				       ;;(printf "~a ~a\n" (car lib) ret )
				       (if (> ret 0)
					   ret
					   (loop (cdr lib))))
