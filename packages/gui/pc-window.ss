...
(library (gui pc-window)
  ...
  (import (scheme) (gui widget) (cffi cffi) (glfw glfw)
    (gui graphic) (utils libutil) (utils macro) (gles gles1))
  (define mouse-x 0)
  (define mouse-y 0)
  ...
  (define (window-cursor-init)
    (hashtable-set!
      cursors
      'arrow
      (glfw-create-standard-cursor GLFW_ARROW_CURSOR))
    (hashtable-set!
      cursors
      'hand
      (glfw-create-standard-cursor GLFW_HAND_CURSOR))
    (hashtable-set!
      cursors
      'ibeam
      (glfw-create-standard-cursor GLFW_IBEAM_CURSOR))
    (hashtable-set!
      cursors
      'crosshair
      (glfw-create-standard-cursor GLFW_CROSSHAIR_CURSOR))
    (hashtable-set!
      cursors
      'hresize
      (glfw-create-standard-cursor GLFW_HRESIZE_CURSOR))
    (hashtable-set!
      cursors
      'vresize
      (glfw-create-standard-cursor GLFW_VRESIZE_CURSOR)))
  (define (window-set-cursor window mod)
    (glfw-set-cursor window (hashtable-ref cursors mod '())))
  (define (window-post-empty-event) (glfw-post-empty-event))
  ...
  (define (window-event-init window)
    ...
    (glfw-set-char-mods-callback
      window
      (lambda (w char mods) (widget-event 5 (vector char mods))))
    (glfw-set-mouse-button-callback
      window
      (lambda (w button action mods)
        (widget-mouse-button-event
          (vector button action mods mouse-x mouse-y))))
    (glfw-set-key-callback
      window
      (lambda (w k s a m) (widget-event 2 (vector k s a m))))
  ...
  (define (collect-thread)
    (if (and (procedure? (top-level-value 'fork-thread))
             (threaded?))
        (try ((top-level-value 'fork-thread)
               (lambda ()
                 (let loop ()
                   (collect)
                   (printf "tid=~a\n" (get-thread-id))
  ...
  (define (window-create width height title)
    (let ([window '()])
      ...
      (printf "~a ,~a ~a,~a\n" (cffi-get-int fb-width)
        (cffi-get-int fb-height) width height)
      (glfw-swap-interval 1)
      (set! ratio (/ (cffi-get-int fb-width) width))
      (window-cursor-init)
      (widget-init-cursor
        (lambda (mod) (window-set-cursor window mod)))
  ...
  (define (window-loop-one window)
    ...
    (if is-show-fps
        (graphic-draw-text
          ...
          fps-y
          (format "fps=~a\n" (graphic-get-fps))))
    (if event-wait-mode (glfw-wait-events) (glfw-poll-events))
    (widget-render)
    (window-run-loop)
    (glfw-swap-buffers window))
